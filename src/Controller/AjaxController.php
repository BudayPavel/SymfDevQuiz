...
class AjaxController extends Controller
{
    ...
    /**
     * @Route("/ajax/{slug}/{action}")
     *
     */
    public function actions(Request $request, $slug, $action, UserPasswordEncoderInterface $passwordEncoder, ValidatorInterface $validator)
    {
        $params = array();
...
//                }
                ...
                switch ($action){
                    case 'add':
                        ...
                        $em->persist($quiz);
                        break;
                    case 'update': $quiz->getQuestions()->clear();
                        $quiz->setname($request->get('text'));
                        ...
                        break;
                    case 'delete': $em->remove($quiz);
                        break;
                    case 'sub':
                        $arr = $em->getRepository(Quiz::class)->findQuestions($params['id']);
                        $out = [];
                        foreach ($arr[0]->getQuestions() as $q)
                        {
